...
function HomeNavigation({ ...allprops }) {
  ...
  if (navigationType === 1) {
    return (
      <>
        <Box
          className={`navigationBar bg-black/[.20] ${styles.navigationBar} ${
            scrollY > 100 ? styles.make_me_dark : ""
          }  flex items-center justify-between pr-8 ${
            mouseOver && "show_border"
          } `}
        >
          <Box className="navigation_bar_left inline-flex">
            <Box
              className="logo_area inline-flex items-center justify-center mr-7 border-r border-c6f6f mt-4 pl-[50px] pr-[30px]  "
              style={{
                height: "70px",
                width: "initial",
              }}
    ...
    return (
      <>
        <Box
          className={` home_navigation_container type2 flex items-center justify-between absolute top-0 left-0 w-full z-50 px-[100px] ${
            mouseOver && "show_border"
          } `}
        >
