...
function index({ props }) {
  ...
  const { asPath } = router
  console.log('currentSlug is',slug)
  let pathString = asPath.replace('/','')
  console.log('pathString',pathString)
  let newTitleString = pathString.replaceAll('-',' ')

  const matched = PlaygroundData.find((c) => c.link === asPath);
      ...
      <>
        ...
        <div className=" pt-10">
          <GalleryHead
            pageLink="/"
            activeItem={"video"}
            bigText={newTitleString ? newTitleString : 'demo'}
            hasCategory={true}
